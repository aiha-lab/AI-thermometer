...
MAIN_THREAD_TIMEOUT = 5.0  # 5 seconds
INPUT_HW = (300, 300)
SUPPORTED_MODELS = [
...
class TrtThread(threading.Thread):
    ...
    """
    def __init__(self, condition, cam, precision, det_time, conf_th):
        """__init__
    ...
    def run(self):
        ...
        while self.running:
            ...
            if img is None:
                break
            boxes, confs, clss, self.det_time = self.trt_ssd.detect(img, self.det_time, self.conf_th)
            with self.condition:
...
def loop_and_display(condition, vis):
        ...
        with condition:
            ...
            if condition.wait(timeout=MAIN_THREAD_TIMEOUT):
                ...
                print("Average FPS : {:.2f} s\n".format(
                    tot_frames / (end_time - start_time-5.0)))
                '''
                break
    ...
    print("Video play time : {:.2f} s\n".format(
        (end_time - start_time - MAIN_THREAD_TIMEOUT)))
    print("Average FPS : {:.2f} s\n".format(
        tot_frames / (end_time - start_time - MAIN_THREAD_TIMEOUT)))
...
def main():
    ...
    trt_thread = TrtThread(condition, cam, args.precision, det_time,  conf_th=0.3)
