...
def _postprocess_trt(img, output, conf_th, det_time, output_layout=7):
    ...
    for prefix in range(0, len(output), output_layout):
        ...
        confs.append(conf)
        clss.append(cls)
    return boxes, confs, clss, det_time
...
class TrtSSD(object):
    ...
    def _load_engine(self):
        TRTbin = 'ssd/wider_%s.buf' % (self.precision)
    ...
    def __init__(self, precision, input_shape, cuda_ctx=None):
    ...
    def detect(self, img, det_time, conf_th=0.3):
        ...
        det_time = det_time + time.time() - det_st_time

        output = self.host_outputs[0]
        return _postprocess_trt(img, output, conf_th, det_time)
