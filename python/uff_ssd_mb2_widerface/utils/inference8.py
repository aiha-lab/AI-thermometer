...
import utils.engine8 as engine_utils # TRT Engine creation/save/load utils
    ...
    def __init__(self, trt_engine_path, uff_model_path, trt_engine_datatype=trt.DataType.FLOAT, batch_size=1):
        ...
        if not os.path.exists(trt_engine_path):
            ...
            if trt_engine_datatype == trt.DataType.INT8:
                ...
                calibration_cache = "ssd_mobilenet_wider_calib.cache"
                
                for n in range(total_imgs):
                    voc_names.append(voc_image_numbers[n] + ".jpg")

                _, calib_data = common.find_sample_data(description="Runs a SSD MobileNet v2 network in Int8 mode", subfolder="JPEGImages", find_files=voc_names)
                calib = WIDEREntropyCalibrator(calib_data, total_imgs, cache_file=calibration_cache)
...
class TensorflowInference(object):
    ...
    def _run_tensorflow_graph(self, image_input):
        ...
        # Output inference time
        print("Tensorflow inference time: {} ms".format(
            int(round((time.time() - inf_start_time) * 1000))))
