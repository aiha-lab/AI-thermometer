...
# Utility functions
import utils.inference8 as inference_utils # TRT/TF inference wrappers
import utils.model as model_utils # UFF conversion
import utils.mAP as voc_mAP_utils # mAP computation
import utils.wider as wider_utils # WIDER FACE dataset descriptors
...
from utils.calibrator import WIDEREntropyCalibrator
import common
...
# WIDER FACE label list
WIDER_LABELS = wider_utils.WIDER_CLASSES_LIST

# Model used for inference
MODEL_NAME = 'mobilenet_lepton_1'
...
def produce_tensorrt_detections(detection_files, trt_inference_wrapper, max_batch_size,
    ...
    for idx in range(0, len(image_numbers), max_batch_size):
        ...
        inf_time_1 = inf_time_1 + time.time() - inf_start_time
            ...
            for det in range(int(keep_count[img_idx])):
                ...
                if confidence > 0.0:
                    ...
                    if label_name:
                        ...
                        # if (img_idx == (len(imgs) - 1)) and (det == (int(keep_count[img_idx])-1)):
        # inf_time_2 = inf_time_2 + time.time() - inf_start_time
                        # detection.write_to_file(det_file)
        
        # print("{:.2f}".format(inf_time))
        # print("{:.2f}".format(inf_time_1))
        # print("{:.2f}".format(inf_time_2))
    ...
    print("Inference time : {:.2f}".format(inf_time_1))
    print("Image / Inf time : {:.2f}".format(total_imgs/(inf_time_1))) 
...
if __name__ == '__main__':
    ...
    if not skip_inference:
        wider_label = WIDER_LABELS[1]
        detection_files[wider_label] = open(
            os.path.join(
                parsed['results_dir'], 'det_test_{}.txt'.format(wider_label)
            ), 'w'
        )
